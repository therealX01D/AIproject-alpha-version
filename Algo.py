...
def multipleBFS(graph, S, Goallist, Queue=[], visited=[], path=[]):
    pathslist=[]
    visitedlist=[]
    for x in range(len(Goallist)):
        print(x)
        paths,visited = BFS(graph,S,Goallist[x],Queue=[],visited=[],path=[])
        print("path to goal ",Goallist[x]," is" ,paths)
        print("visited to goal ",Goallist[x]," is", visited)
        pathslist.append(paths)
        visitedlist.append(visited)
    pathslist.sort(key=len)
    visitedlist.sort(key=len)
    print(pathslist, visitedlist)
    return pathslist[0],visitedlist[0]    
